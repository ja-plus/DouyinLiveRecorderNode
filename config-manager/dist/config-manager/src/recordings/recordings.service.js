...
export function getThumbsDir(baseDir) {
    return path.join(baseDir, ".thumbs");
}
export function thumbFileName(fileRel) {
    // 用相对路径的短 hash 作为缩略图文件名，避免原文件名过长或含特殊字符。
    const hash = crypto.createHash("sha256").update(fileRel.split(path.sep).join("/")).digest("hex").slice(0, 16);
    return `${hash}.jpg`;
}
export function getThumbPath(baseDir, fileRel) {
    return path.join(getThumbsDir(baseDir), thumbFileName(fileRel));
}
export function hasThumbnail(baseDir, fileRel) {
    return fs.existsSync(getThumbPath(baseDir, fileRel));
}
export function deleteThumbnail(baseDir, fileRel, logger) {
...
export async function generateThumbnail(baseDir, fileRel, logger) {
...
function cleanupOrphanThumbs(baseDir, expectedNames, logger) {
    const thumbsDir = getThumbsDir(baseDir);
    if (!fs.existsSync(thumbsDir)) return;
    let entries;
    try {
        entries = fs.readdirSync(thumbsDir, {
            withFileTypes: true
        });
    } catch (err) {
        logger.warn({
...
export function listRecordings(baseDir, anchorName, logger) {
    // 递归扫描录制输出目录，因为主播文件可能位于子目录中。
    const files = [];
    const expectedThumbNames = new Set();
    const walk = (dir)=>{
        let entries;
        try {
        ...
        for (const entry of entries){
            ...
            const ext = path.extname(entry.name).toLowerCase(), relative = path.relative(baseDir, fullPath);
            if (!VIDEO_EXTS.has(ext) || !(entry.name.startsWith(`${anchorName}_`) || relative.split(path.sep).slice(0, -1).includes(anchorName))) continue;
            try {
...
export function resolveRecordingPath(baseDir, file) {
    // 先解析并校验路径范围，防止路径穿越到下载目录之外。
    const fullPath = path.resolve(baseDir, file);
    if (fullPath === baseDir || !fullPath.startsWith(`${baseDir}${path.sep}`)) return {
        error: "非法路径"
    };
    if (!VIDEO_EXTS.has(path.extname(fullPath).toLowerCase())) return {
        error: "不支持的文件类型"
    };
...
export function deleteRecording(baseDir, file, logger) {
