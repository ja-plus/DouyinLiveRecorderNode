...
import { checkPortAvailable, getServerSettings, loadTlsOptions } from "./common/settings.js";
...
async function killStaleInstance(logger) {
    let pid;
    try {
...
export async function startServer(options = {}) {
    ...
    if (loadError) logger.warn({
        err: loadError
    }, "config.js 加载失败，使用默认配置");
    logger.info({
        configPath: CONFIG_PATH
    }, "配置文件路径");
    // 显式传入的选项优先于从 config.js 读取的配置。
    const host = options.host ?? settings.host, port = options.port ?? settings.port;
    ...
    if (!portCheck.ok) throw new Error(portCheck.reason === "EADDRINUSE" ? `端口 ${port} 已被其他程序占用` : `端口检测失败(${portCheck.reason})：${host}:${port}`);
    const http2 = (options.http2 ?? settings.enableHttp2) && !!tls;
    if ((options.http2 ?? settings.enableHttp2) && !tls) logger.warn("enableHttp2=true 但未找到 TLS 证书，已回退 HTTP/1.1。");
    ...
    const nestApp = await NestFactory.create(createAppModule(auth.AuthModule, settings.recorderStatusUrl, logger, settings.sqliteLogPath), adapter, {
        logger: new NestPinoLogger(logger)
    });
    // 静态/媒体资源请求日志降级，避免高频资源请求刷屏。
    app.addHook("onRequest", (request, _reply, done)=>{
        const url = request.url.split("?")[0];
        if (url.startsWith("/api/video/") || /\.(?:js|css|map|png|jpe?g|gif|svg|ico|woff2?|ttf|mp4|flv|ts|mkv|mp3|m4a)$/i.test(url)) request.log.level = "warn";
        done();
    });
