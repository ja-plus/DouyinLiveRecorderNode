...
export async function registerStaticRoutes(app, logger) {
    ...
    if (fs.existsSync(downloads)) await app.register(fastifyStatic, {
        root: downloads,
        prefix: "/api/video/",
        decorateReply: false
    });
    ...
    app.get("/*", (request, reply)=>{
        ...
        if (!fs.existsSync(index)) logger.warn("前端未构建：缺少 static/index.html");
        return fs.existsSync(index) ? reply.type("text/html").send(fs.readFileSync(index)) : reply.code(404).send({
            success: false,
