...
export class ConfigController {
    ...
    saveUrlConfig(body, reply) {
        try {
            if (!Array.isArray(body?.items)) return this.failed(reply, 400, "items 必须是数组");
    ...
    saveAppConfig(body, reply) {
        try {
            if (!Array.isArray(body?.sections)) return this.failed(reply, 400, "sections 必须是数组");
            saveConfig(APP_CONFIG_PATH, // 仅替换已知键的值，保留注释和未知的 INI 内容。
